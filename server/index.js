...
app.post('/api/analyze', async (req, res) => {
  try {
    ...
    const response = await openai.chat.completions.create({
        model: "qwen/qwen3-vl-30b-a3b-thinking", // Отличная и дешевая модель для старта. 
      ...
      messages: [
        {
          ...
          content: [
            { 
              type: "text", 
              text: `YOU ARE A TOP IMAGE CONSULTANT AND FASHION ANALYST, 
WINNER OF THE INTERNATIONAL STYLING EXCELLENCE AWARD 2024.

ТВОЯ ЗАДАЧА — КРАТКО И ПРОФЕССИОНАЛЬНО ОЦЕНИТЬ ОБРАЗ ЧЕЛОВЕКА ПО ФОТО ИЛИ ОПИСАНИЮ.

АЛГОРИТМ АНАЛИЗА:

1. Определи стиль (1–2 слова).
2. Оцени цветовую гармонию (удачно / спорно / неудачно + краткое пояснение).
3. Оцени посадку и пропорции (что работает / что нарушено).
4. Назови 1–2 сильные стороны.
5. Назови 1–2 конкретные рекомендации по улучшению.
6. Поставь оценку по шкале 1–10 с кратким обоснованием (1 предложение).
...
WHAT NOT TO DO:
- НЕ ОПИСЫВАЙ ВНЕШНОСТЬ ЧЕЛОВЕКА.
- НЕ ИСПОЛЬЗУЙ РАЗМЫТЫЕ ФРАЗЫ.
- НЕ ПИШИ ДЛИННЫХ АБЗАЦЕВ.
- НЕ ДЕЛАЙ СОЦИАЛЬНЫХ ПРЕДПОЛОЖЕНИЙ.

ОТВЕТ ДОЛЖЕН БЫТЬ КОНКРЕТНЫМ, ЧЕТКИМ И ДО 10–12 ПРЕДЛОЖЕНИЙ.` 
            },
            {
              type: "image_url",
              image_url: { url: image },
